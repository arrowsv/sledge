...
namespace rfg {
    ...
    //     *this_, char const *tag_name); uint8_t(__thiscall *const get_tag_world_coords)(class
    //     object const *this_, int32_t tag_index, class vector *tag_pos, class matrix *tag_orient,
    //     class vector *obj_pos, class matrix *obj_orient); uint8_t(__thiscall *const
    //     get_tag_local_coords)(class object const *this_, int32_t tag_index, class vector
    //     *tag_pos, class matrix *tag_orient); class rl_renderable_mesh const *(__thiscall *const
    ...
    //     disable_object_outline)(class object *this_); int32_t(__thiscall *const
    //     attach_index)(class object *this_, class vector const *_pos, uint32_t
    //     parent_objp_body_handle); uint8_t(__thiscall *const attach_index_is_valid)(class object
    //     *this_, int32_t index); float(__thiscall *const get_mass)(class object const *this_);
    ...
    //     uint8_t(__thiscall *const get_effect_material)(class object const *this_, uint32_t
    //     shape_key, class vector *contact_point, class hkpWorldObject const *hk_objp);
    //     float(__thiscall *const get_energy_scale)(class object *this_); void(__thiscall *const
    //     process_bullet_hit)(class object *this_, class human *attacker, float damage, struct
    //     weapon_fire_info const *fire_info); uint8_t(__thiscall *const process_melee_hit)(class
    //     object *this_, class object *attacker, float damage, class vector const *impulse, class
    //     vector const *hit_pos, class vector const *hit_normal, uint32_t shape_key, struct
    //     rfg::weapon_info const *w_info); void(__thiscall *const hierarchy_notify)(class object
    //     *this_, enum hierarchy_notify_code code); uint8_t(__thiscall *const
    //     ready_to_stream_out)(class object *this_); uint32_t(__thiscall *const
    //     get_alt_hk_body_handle)(class object const *this_, uint32_t index); uint32_t(__thiscall
    //     *const get_alt_hk_body_handle_index)(class object const *this_, uint32_t alt_handle);
    //     int32_t (*const j___purecall_1)();
    // };
    ...
    // REF_FUNC(havok_body_apply_linear_impulse, void __cdecl(uint32_t handle, const rfg::vector*
    // impulse),
    //          0x00823cb0);
    // REF_FUNC(havok_body_apply_point_impulse,
    //          void __cdecl(uint32_t handle, const rfg::vector* impulse, const rfg::vector*
    //          position), 0x00823d30);
}
