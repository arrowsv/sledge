...
namespace lua::bindings::rfg {
    void bind_vector(sol::state_view& lua) {
        ...
        auto vector = types.new_usertype<::rfg::vector>(
            "vector",
            sol::constructors<::rfg::vector(), ::rfg::vector(const ::rfg::vector&),
        ...
        vector[sol::meta_function::subtraction] = [](const ::rfg::vector& a,
                                                     const ::rfg::vector& b) {
            return ::rfg::vector(a.x - b.x, a.y - b.y, a.z - b.z);
        };
        ...
        vector[sol::meta_function::division] =
            sol::overload([](const ::rfg::vector& v, float scalar) {
                return ::rfg::vector(v.x / scalar, v.y / scalar, v.z / scalar);
            });
    }
    ...
    void bind_cubic_bezier_spline(sol::state_view& lua) {
        auto types = lua["types"].get_or_create<sol::table>();
        auto cubic_bezier_spline = types.new_usertype<::rfg::cubic_bezier_spline>(
            "cubic_bezier_spline", sol::no_constructor);
