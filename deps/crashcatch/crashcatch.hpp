...
inline std::string demangle(const char *symbol)
{
    ...
    if (parenOpen != std::string::npos && plusSign != std::string::npos && plusSign > parenOpen + 1)
    {
        ...
        {
            std::string result = sym.substr(0, parenOpen + 1) + demangled + sym.substr(plusSign);
            free(demangled);
            return result;
        }
        if (demangled)
...
inline void writeCrashLog(const std::string &logPath, const std::string &timestamp, int signal = 0)
{
    ...
    if (ec)
        return; // can't create output directory — bail out silently
...
inline LONG WINAPI UnhandledExceptionHandler(EXCEPTION_POINTERS *ep)
{
    ...
    if (ec)
        return EXCEPTION_EXECUTE_HANDLER; // can't create output directory
    ...
    if (hFile != INVALID_HANDLE_VALUE)
    {
        ...
        MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), hFile, MiniDumpWithDataSegs, &dumpInfo, nullptr,
                          nullptr);
        CloseHandle(hFile);
