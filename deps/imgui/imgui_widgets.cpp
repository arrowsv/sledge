...
bool ImGui::ButtonBehavior(const ImRect &bb, ImGuiID id, bool *out_hovered, bool *out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.DragDropActive)
    {
        if ((flags & ImGuiButtonFlags_PressedOnDragDropHold) &&
    ...
    if (hovered)
    {
        ...
        if (mods_ok)
        {
            if (mouse_button_clicked != -1 && g.ActiveId != id)
            {
                ...
                {
                    // FIXME: Traditionally ImGuiButtonFlags_PressedOnRelease never took ActiveId. Adding it in
                    // 2026-03-20 since ImGuiButtonFlags_NoHoldingActiveId can always be added. We don't yet perform an
                    // explicit ClearActiveID() to reduce scope of change, but this possibility could be investigated.
                    if (!(flags & ImGuiButtonFlags_NoHoldingActiveId))
                        SetActiveID(id, window); // Hold on ID
                    g.ActiveIdMouseButton = (ImS8)mouse_button_clicked;
                }
    ...
    if ((item_flags & ImGuiItemFlags_Disabled) == 0)
    {
        ...
        if (g.NavActivateDownId == id)
        {
            ...
            {
                // Set active id so it can be queried by user via IsItemActive(), equivalent of holding the mouse
                // button.
                pressed = true;
                SetActiveID(id, window);
...
bool ImGui::ButtonEx(const char *label, const ImVec2 &size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, label_end, &label_size,
...
bool ImGui::InvisibleButton(const char *str_id, const ImVec2 &size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 size = CalcItemSize(
        ImVec2(size_arg.x != 0.0f ? size_arg.x : -FLT_MIN, size_arg.y != 0.0f ? size_arg.y : -FLT_MIN), 0.0f, 0.0f);
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow *window, ImGuiAxis axis)
{
    ...
    const float border_top = (window->Flags & ImGuiWindowFlags_MenuBar)      ? IM_ROUND(g.Style.FrameBorderSize * 0.5f)
                             : (window->Flags & ImGuiWindowFlags_NoTitleBar) ? border_size
                                                                             : 0;
    if (axis == ImGuiAxis_X)
...
void ImGui::ExtendHitBoxWhenNearViewportEdge(ImGuiWindow *window, ImRect *bb, float threshold, ImGuiAxis axis)
{
    ...
    if (window_rect.Min[axis] == viewport_rect.Min[axis] && bb->Min[axis] > window_rect.Min[axis] &&
        bb->Min[axis] - threshold <= window_rect.Min[axis])
        bb->Min[axis] = window_rect.Min[axis];
    if (window_rect.Max[axis] == viewport_rect.Max[axis] && bb->Max[axis] < window_rect.Max[axis] &&
        bb->Max[axis] + threshold >= window_rect.Max[axis])
                        ...
                        ImS64 size_contents_v, ImDrawFlags draw_rounding_flags)
{
    ...
    const float grab_h_pixels = (float)(int)ImClamp(scrollbar_size_v * ((float)size_visible_v / (float)win_size_v),
                                                    grab_h_minsize, scrollbar_size_v);
    const float grab_h_norm = grab_h_pixels / scrollbar_size_v;

    // As a special thing, we allow scrollbar near the edge of a screen/viewport to be reachable with mouse at the
    // extreme edge (#9276)
                        ...
                        const ImVec4 &bg_col, const ImVec4 &tint_col)
{
    ...
    if (rounding > 0.0f)
        window->DrawList->AddImageRounded(tex_ref, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColorU32(tint_col),
                                          rounding);
    else
...
bool ImGui::Checkbox(const char *label, bool *v)
{
    ...
    if (is_visible)
    {
        ...
        ImU32 bg_col = GetColorU32((held && hovered)          ? ImGuiCol_FrameBgActive
                                   ...
                                   : (mixed_value || checked) ? ImGuiCol_CheckboxSelectedBg
                                                              : ImGuiCol_FrameBg);
...
void ImGui::ProgressBar(float fraction, const ImVec2 &size_arg, const char *overlay)
{
    ...
    if (fill_x0 < fill_x1)
        RenderRectFilledInRangeH(window->DrawList, bb, GetColorU32(ImGuiCol_PlotHistogram), fill_x0, fill_x1,
                                 style.FrameRounding);
...
bool ImGui::TextLink(const char *label)
{
    ...
    window->DrawList->AddLineH(bb.Min.x, bb.Max.x, line_y, GetColorU32(line_colf),
                               1.0f * (float)(int)g.Style._MainScale); // FIXME-TEXT: Underline mode // FIXME-DPI
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        // This was mostly chosen to allow backward compatibility with user's code assuming zero-height when calculating
        // height for layout (e.g. bottom alignment of a status bar). In order to handle scaling we need to scale
...
bool ImGui::BeginCombo(const char *label, const char *preview_value, ImGuiComboFlags flags)
{
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(bb.Max.x + style.ItemInnerSpacing.x, bb.Min.y + style.FramePadding.y), label, label_end,
                   false);
...
static const ImU32 GDefaultRgbaColorMarkers[4] = {IM_COL32(240, 20, 20, 255), IM_COL32(20, 240, 20, 255),
                                  ...
                                  void *p_data_when_empty)
{
    ...
    else
    {
        ...
        if (format[0] == '\0')
            format = type_info->ScanFmt; // Format doesn't want us to show the number currently, but we still need to
                                         // parse the resulting input
    }
...
static float GetMinimumStepAtDecimalPrecision(int decimal_precision)
{
    ...
    if (decimal_precision < 0)
        return FLT_MIN;
    return (decimal_precision < IM_COUNTOF(min_steps)) ? min_steps[decimal_precision]
                          ...
                          const char *format, ImGuiSliderFlags flags)
{
    ...
    if (is_logarithmic)
    {
        ...
        float v_old_parametric = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(
            data_type, v_cur, v_min, v_max, logarithmic_zero_epsilon, zero_deadzone_halfsize);
        float v_new_parametric = v_old_parametric + g.DragCurrentAccum;
        v_cur = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, v_new_parametric, v_min, v_max,
...
static bool TempInputIsClampEnabled(ImGuiSliderFlags flags, ImGuiDataType data_type, const void *p_min,
                                    const void *p_max)
{
    if ((flags & ImGuiSliderFlags_ClampOnInput) && (p_min != NULL || p_max != NULL))
    {
        const int clamp_range_dir = (p_min != NULL && p_max != NULL)
                                        ? ImGui::DataTypeCompare(data_type, p_min, p_max)
            ...
            else
            {
                ...
                if (g.ActiveIdIsJustActivated)
                {
                    float grab_t = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(
                        data_type, *v, v_min, v_max, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                    if (axis == ImGuiAxis_Y)
                        grab_t = 1.0f - grab_t;
            ...
            else if (g.SliderCurrentAccumDirty)
            {
                ...
                else
                {
                    ...
                    TYPE v_new = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(
                        data_type, clicked_t, v_min, v_max, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                    if (is_floating_point && !(flags & ImGuiSliderFlags_NoRoundToFormat))
                        v_new = RoundScalarWithFormatT<TYPE>(format, data_type, v_new);
                    float new_clicked_t = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(
                        data_type, v_new, v_min, v_max, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                          ...
                          ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void *user_data)
{
    ...
    bool value_changed =
        InputTextEx(label, NULL, buf, (int)buf_size, bb.GetSize(),
                    flags | ImGuiInputTextFlags_TempInput | ImGuiInputTextFlags_AutoSelectAll, callback, user_data);
                        ...
                        const void *p_step_fast, const char *format, ImGuiInputTextFlags flags)
{
    ...
    ImGuiContext &g = *GImGui;
    ImGuiStyle &style = g.Style;
    // IM_ASSERT((flags & ImGuiInputTextFlags_EnterReturnsTrue) == 0); // Not supported by InputScalar(). Please open an
    // issue if you this would be useful to you. Otherwise use IsItemDeactivatedAfterEdit()!
    ...
    if (has_step_buttons)
    {
        ...
        ret = InputText("", buf, IM_COUNTOF(buf),
    ...
    bool value_changed =
        input_edited ? DataTypeApplyFromText(buf, data_type, p_data, format,
...
static int STB_TEXTEDIT_MOVEWORDLEFT_IMPL(ImGuiInputTextState *obj, int idx)
{
    ...
    while (idx >= 0 && !is_word_boundary_from_right(obj, idx)) idx = IMSTB_TEXTEDIT_GETPREVCHARINDEX(obj, idx);
    return idx < 0 ? 0 : idx;
}
static int STB_TEXTEDIT_MOVEWORDRIGHT_MAC(ImGuiInputTextState *obj, int idx)
{
    ...
    while (idx < len && !is_word_boundary_from_left(obj, idx)) idx = IMSTB_TEXTEDIT_GETNEXTCHARINDEX(obj, idx);
...
static int STB_TEXTEDIT_MOVEWORDRIGHT_WIN(ImGuiInputTextState *obj, int idx)
{
    idx = IMSTB_TEXTEDIT_GETNEXTCHARINDEX(obj, idx);
    int len = obj->TextLen;
    while (idx < len && !is_word_boundary_from_right(obj, idx)) idx = IMSTB_TEXTEDIT_GETNEXTCHARINDEX(obj, idx);
    return idx > len ? len : idx;
}
...
static int STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState *obj, int pos, const char *new_text, int new_text_len)
{
    ...
    if (!is_resizable && new_text_len > avail)
        new_text_len =
            (int)(ImTextFindValidUtf8CodepointEnd(new_text, new_text + new_text_len, new_text + avail) -
                  new_text); // Truncate to closest UTF-8 codepoint. Alternative: return 0 to cancel insertion.
    if (new_text_len == 0)
        return 0;
...
void ImGuiInputTextState::SetSelection(int start, int end)
{
    Stb->select_start = start;
    Stb->cursor = Stb->select_end = end;
}
                        ...
                        ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void *callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        bool no_clip =
            ...
            (id == g.NavActivateId); // Mimic some of ItemAdd() logic + add InputTextDeactivatedState.ID check.
    ...
    const bool user_scroll_finish =
        is_multiline && state != NULL && g.ActiveId == 0 && g.ActiveIdPreviousFrame == scrollbar_id;
    ...
    const bool init_changed_specs_multiline =
        (state != NULL && (state->Stb->single_line != !is_multiline)); // state != NULL means its our state.
    const bool init_changed_specs_readonly =
        (state != NULL &&
         ((state->Flags ^ flags) & ImGuiInputTextFlags_ReadOnly)); // state != NULL means its our state.
    const bool init_make_active =
        (input_requested_by_user || input_requested_by_nav || input_requested_by_reactivate || user_scroll_finish);
    ...
    else if ((init_make_active && g.ActiveId != id) || init_changed_specs_multiline || init_changed_specs_readonly)
    {
        ...
        if (recycle_state && !init_changed_specs_readonly &&
    ...
    if (g.ActiveId == id)
    {
        ...
        const ImGuiKey always_owned_keys[] = {ImGuiKey_LeftArrow, ImGuiKey_RightArrow, ImGuiKey_Delete,
                                              ImGuiKey_Backspace, ImGuiKey_Home,       ImGuiKey_End};
    ...
    if (state != NULL && state->ID == id)
    {
        ...
        // Word-wrapping: attempt to keep cursor in view while resizing frame/parent (FIXME-WORDWRAP: would be better to
        // preserve same relative offset)
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_ctrl_enter = Shortcut(ImGuiMod_Ctrl | ImGuiKey_Enter, f_repeat, id) ||
                                   Shortcut(ImGuiMod_Ctrl | ImGuiKey_KeypadEnter, f_repeat, id);
        const bool is_shift_enter = Shortcut(ImGuiMod_Shift | ImGuiKey_Enter, f_repeat, id) ||
                                    Shortcut(ImGuiMod_Shift | ImGuiKey_KeypadEnter, f_repeat, id);
        ...
        else if (is_enter || is_ctrl_enter || is_shift_enter || is_gamepad_validate)
        {
            ...
            bool is_new_line = is_multiline && !is_gamepad_validate &&
                               (is_shift_enter || (is_enter && !ctrl_enter_for_new_line) ||
                                (is_ctrl_enter && ctrl_enter_for_new_line));
            if (!is_new_line)
            {
            ...
            if (event_flag)
            {
                ...
                if (callback_data.BufDirty)
                {
                    ...
                    IM_ASSERT(
                        ...
                        (int)ImStrlen(callback_data.Buf)); // You need to maintain BufTextLen if you change the text!
    ...
    // This is used when e.g. losing focus or tabbing out into another InputText() which may already be using the temp
    // buffer.
    ...
    // Write back result to user buffer. This can currently only happen when (g.ActiveId == id) or when just
    // deactivated.
    // - As soon as the InputText() is active, our stored in-widget value gets priority over any underlying modification
    // of the user buffer.
    // - Make sure we always reapply the live buffer back to the input/user buffer before clearing ActiveId, even
    // thought strictly speaking
    //   it was not modified on this frame. This allows the user to use InputText() without maintaining any user-side
    //   storage. (PS: if you use this property together with ImGuiInputTextFlags_CallbackResize, you are at the risk of
    //   recreating a temporary
    //    allocated/string object every frame. Which in the grand scheme of scheme is nothing, but isn't dear imgui
    //    vibe).
    ...
    if (is_multiline)
    {
        ...
        // FIXME-OPT: Could append to index when new value of line_visible_n1 becomes bigger, see second call to
        // CalcClipRectVisibleItemsY() below.
        bool will_scroll_y = state && ((state->CursorFollow && render_cursor) ||
                                       (state->CursorCenterY && (render_cursor || render_selection)));
        line_count = InputTextLineIndexBuild(flags, line_index, buf_display, buf_display_end, wrap_width,
                                             will_scroll_y ? INT_MAX : line_visible_n1 + 1,
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            const float bg_offy_up =
                is_multiline ? 0.0f : -1.0f; // FIXME-DPI: those offsets should be part of the style? they don't play so
    ...
    if ((is_multiline || (buf_display_end - buf_display) < buf_display_max_length) && (text_col & IM_COL32_A_MASK) &&
        ...
        g.Font->RenderText(draw_window->DrawList, g.FontSize,
                           draw_pos - draw_scroll + ImVec2(0.0f, line_visible_n0 * g.FontSize), text_col,
    ...
    if (render_cursor)
    {
        ...
        if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
            draw_window->DrawList->AddLineV(cursor_screen_rect.Min.x, cursor_screen_rect.Min.y,
                                            cursor_screen_rect.Max.y, GetColorU32(ImGuiCol_InputTextCursor),
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags)
{
        ...
        (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const bool draw_color_marker =
            (flags & (ImGuiColorEditFlags_DisplayHSV | ImGuiColorEditFlags_NoColorMarkers)) == 0;

        const bool hide_prefix =
            draw_color_marker ||
            (w_per_component <= CalcTextSize((flags & ImGuiColorEditFlags_Float) ? "M:0.000" : "M:000").x);
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            if (flags & ImGuiColorEditFlags_Float)
            {
                value_changed |= DragFloat(ids[n], &f[n], 1.0f / 255.0f, 0.0f, hdr ? 0.0f : 1.0f,
                                           fmt_table_float[fmt_idx][n], drag_flags);
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        char buf[64];
        if (alpha)
            ImFormatString(buf, IM_COUNTOF(buf), "#%02X%02X%02X%02X", ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255),
                           ImClamp(i[2], 0, 255), ImClamp(i[3], 0, 255));
        else
            ImFormatString(buf, IM_COUNTOF(buf), "#%02X%02X%02X", ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255),
                           ImClamp(i[2], 0, 255));
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(
                bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), rounding,
                1.0f * (float)(int)
                           g.Style._MainScale); // Color buttons are often in need of some sort of border // FIXME-DPI
    }
...
void ImGui::ColorEditOptionsPopup(const float *col, ImGuiColorEditFlags flags)
{
    ...
    if (BeginPopup("Copy"))
    {
        int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]), cb = IM_F32_TO_INT8_SAT(col[2]),
            ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
        char buf[64];
        ImFormatString(buf, IM_COUNTOF(buf), "(%.3ff, %.3ff, %.3ff, %.3ff)", col[0], col[1], col[2],
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label, const char *label_end)
{
    ...
    const ImVec2 padding = use_frame_padding ? style.FramePadding
...
bool ImGui::Selectable(const char *label, bool selected, ImGuiSelectableFlags flags, const ImVec2 &size_arg)
{
    ...
    if (is_visible)
        RenderTextClipped(pos, ImVec2(ImMin(pos.x + size.x, window->WorkRect.Max.x), pos.y + size.y), label, label_end,
                          &label_size, style.SelectableTextAlign, &bb);

#ifdef IMGUI_DEBUG_BOXSELECT
    if (g.BoxSelectState.UnclipMode)
    {
        GetForegroundDrawList()->AddText(pos, IM_COL32(255, 255, 0, 200), label, label_end);
    }
#endif
                           ...
                           ImGuiMultiSelectFlags ms_flags)
{
    ...
    // bs->StartPosRel.y, bs->EndPosRel.x, bs->EndPosRel.y, WindowPosAbsToRel(window, g.IO.MousePos).x,
    // WindowPosAbsToRel(window, g.IO.MousePos).y);

    // Box-select 2D mode detects change of the rectangle.
    // Storing unclip rects which will be tested by widgets supporting box-select. Always update rectangles when active
    // (even if we don't use them). To facilitate understanding this: enable IMGUI_DEBUG_BOXSELECT and visualize all
    // geometry.
    if (ms_flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d))
    {
        ...
        {
            ImVec2 d_min = (side == 0) ? ImMin(bs->BoxSelectRectCurr.Min, bs->BoxSelectRectPrev.Min)
                                       : ImMin(bs->BoxSelectRectCurr.Max, bs->BoxSelectRectPrev.Max);
            ImVec2 d_max = (side == 0) ? ImMax(bs->BoxSelectRectCurr.Min, bs->BoxSelectRectPrev.Min)
        ...
        if (ms_flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d))
            if (bs->BoxSelectRectPrev.Min.y != bs->BoxSelectRectCurr.Min.y ||
                bs->BoxSelectRectPrev.Max.y != bs->BoxSelectRectCurr.Max.y)
            {
        ...
        if (!bs->UnclipRect.IsInverted() &&
            (!window->ClipRect.Contains(bs->UnclipRect.Min) ||
             !window->ClipRect.Contains(bs->UnclipRect.Max))) // !! Don't use Contains(ImRect)
            bs->UnclipMode = true;
    ...
    if (ms_flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d))
    {
        for (ImRect &unclip_r : bs->UnclipRects)
            if (!unclip_r.IsInverted())
                GetForegroundDrawList()->AddRect(unclip_r.Min, unclip_r.Max,
                                                 bs->UnclipMode ? IM_COL32(255, 255, 0, 200) : IM_COL32(255, 0, 0, 200),
                                                 0.0f, 0, 4.0f);
        GetForegroundDrawList()->AddRect(bs->UnclipRect.Min, bs->UnclipRect.Max,
                                         bs->UnclipMode ? IM_COL32(255, 255, 0, 200) : IM_COL32(255, 0, 0, 200), 0.0f,
                                         0, 2.0f);
    }
#endif
    return true;
...
void ImGui::EndBoxSelect(const ImRect &scope_rect, ImGuiMultiSelectFlags ms_flags)
{
    ...
    draw_window->DrawList->AddRectFilled(box_select_r.Min, box_select_r.Max,
                                         GetColorU32(ImGuiCol_SeparatorHovered, 0.30f)); // FIXME-MULTISELECT: Styling
    draw_window->DrawList->AddRect(box_select_r.Min, box_select_r.Max,
...
ImGuiMultiSelectIO *ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
    ...
    if (flags & ImGuiMultiSelectFlags_ScopeRect)
        window->DC.CursorMaxPos =
            ms->ScopeRectMin; // CalcScopeRect() for ImGuiMultiSelectFlags_ScopeRect will measure in EndMultiSelect().
...
void ImGui::SetNextItemSelectionUserData(ImGuiSelectionUserData selection_user_data)
{
    ...
    if (ImGuiMultiSelectTempData *ms = g.CurrentMultiSelect)
    {
        ...
        // ms->PrevSubmittedItem = ms->CurrSubmittedItem; // Can't rely on previous g.NextItemData.SelectionUserData
        // because NextItemData is not restored on nested multi-select. ms->CurrSubmittedItem = selection_user_data;
    }
    else
    {
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool *p_selected, ImGuiButtonFlags *p_button_flags)
{
    ...
    if (p_button_flags != NULL)
    {
        ...
        if (ms->Flags & ImGuiMultiSelectFlags_SelectOnClickAlways)
            ...
            button_flags |= (!selected || (g.ActiveId == id && g.ActiveIdHasBeenPressedBefore))
...
void ImGui::MultiSelectItemFooter(ImGuiID id, bool *p_selected, bool *p_pressed)
{
            ...
            if (!window->DC.NavIsScrollPushableX) // FIXME: Rename to be more generic.
                if (ImGuiTable *table = g.CurrentTable)
                    if (table->CurrentColumn != -1)
                    {
                        ...
                        // However we account for ClipRect being larger than current column (e.g. when using
                        // SpanAllColumns) A more generic version would be nice, but window->WorkRect.Min/Max exclude
                        // CellPadding. (#7994, #9383)
                        ImGuiTableColumn *column = &table->Columns[table->CurrentColumn];
                        float clip_min_x = (g.LastItemData.ItemFlags & ImGuiItemStatusFlags_HasClipRect)
                                               ? g.LastItemData.ClipRect.Min.x
                                               : window->ClipRect.Min.x;
                        float clip_max_x = (g.LastItemData.ItemFlags & ImGuiItemStatusFlags_HasClipRect)
                                               ? g.LastItemData.ClipRect.Max.x
                                               : window->ClipRect.Max.x;
                        if (clip_min_x != clip_max_x) // When zero sized we expect that bounds have been clamped and
                                                      // thus are unreliable
                        {
...
#ifdef IMGUI_DEBUG_BOXSELECT
                    GetForegroundDrawList()->AddRectFilled(g.LastItemData.Rect.Min, g.LastItemData.Rect.Max,
                                                           selected ? IM_COL32(0, 255, 0, 200)
    ...
    if (hovered && IsMouseClicked(1) &&
        (flags & (ImGuiMultiSelectFlags_NoAutoSelect | ImGuiMultiSelectFlags_NoSelectOnRightClick)) == 0)
    {
    ...
    if (pressed && (!enter_pressed || !selected))
    {
        ...
        if (flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d))
            if (!g.BoxSelectState.IsActive && !g.BoxSelectState.IsStarting && input_source == ImGuiInputSource_Mouse &&
                                   ...
                                   ImGuiSelectionUserData first_item, ImGuiSelectionUserData last_item)
{
    ...
    // FIXME-OPT: Disabled on 2026/04/09 as this would break with any form of coarse clipping that we don't know about
    // (e.g. TableNextColumn() return value). The low-hanging fruit would be to know that ImGuiSelectionUserData are
    // sequential indices, in which case we can trivially compare PrevSubmittedItem + RangeDir == FirstItem. User can
    // always perform this merge if required.
#if 0
...
void ImGuiSelectionExternalStorage::ApplyRequests(ImGuiMultiSelectIO *ms_io)
{
    ...
    {
        if (req.Type == ImGuiSelectionRequestType_SetAll)
            for (int idx = 0; idx < ms_io->ItemsCount; idx++) AdapterSetItemSelected(this, idx, req.Selected);
        if (req.Type == ImGuiSelectionRequestType_SetRange)
            for (int idx = (int)req.RangeFirstItem; idx <= (int)req.RangeLastItem; idx++)
                  ...
                  const ImVec2 &size_arg)
{
    ...
    if (values_count >= values_count_min)
    {
        ...
        {
            const float t =
                ImClamp((g.IO.MousePos.x - inner_bb.Min.x) / (inner_bb.Max.x - inner_bb.Min.x), 0.0f, 0.999f);
...
bool ImGui::BeginMenuBar()
{
    ...
    ImRect clip_rect(ImFloor(bar_rect.Min.x + border_half), ImFloor(bar_rect.Min.y + border_top),
...
void ImGui::EndMainMenuBar()
{
    ...
    IM_ASSERT_USER_ERROR_RET(
        g.CurrentWindow->DC.MenuBarAppending,
        "Calling EndMainMenuBar() not from a menu-bar!"); // Not technically testing that it is the main menu bar
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled)
{
    ...
    const ImGuiSelectableFlags selectable_flags =
        ImGuiSelectableFlags_NoAutoClosePopups | (ImGuiSelectableFlags)ImGuiSelectableFlags_SelectOnClick;
    ...
    else
    {
        ...
        RenderArrow(window->DrawList,
                    ImVec2(text_pos.x + offsets->OffsetMark + extra_w + g.FontSize * 0.30f, text_pos.y),
    ...
    // Once dragged, release ActiveId + key ownership. This is to allow the idiom of mouse down a menu, dragging
...
void ImGui::EndMenu()
{
    ...
    ImGuiContext &g = *GImGui;
    ImGuiWindow *window = g.CurrentWindow;
    IM_ASSERT_USER_ERROR_RET((window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) ==
                                 (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu),
                             "Calling EndMenu() in wrong window!");
...
bool ImGui::MenuItemEx(const char *label, const char *icon, const char *shortcut, bool selected, bool enabled)
{
    ...
    const ImGuiSelectableFlags selectable_flags = (ImGuiSelectableFlags)ImGuiSelectableFlags_SelectOnRelease |
    ...
    else
    {
        ...
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
        {
            ...
            if (selected)
                RenderCheckMark(
                    window->DrawList,
                    text_pos + ImVec2(offsets->OffsetMark + stretch_w + g.FontSize * 0.40f, g.FontSize * 0.134f * 0.5f),
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar)
{
    ...
    sections[0].Spacing =
        sections[0].TabCount > 0 && (sections[1].TabCount + sections[2].TabCount) > 0 ? tab_spacing : 0.0f;
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        section->WidthAfterShrinkMinWidth +=
            ImMin(tab->ContentWidth, shrink_min_width) + (section_n == curr_section_n ? tab_spacing : 0.0f);
...
void ImGui::TabBarAddTab(ImGuiTabBar *tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow *window)
{
    ...
    IM_ASSERT(g.CurrentTabBar !=
              tab_bar); // Can't work while the tab bar is active as our tab doesn't have an X offset yet, in theory we
                        ...
                        // solve why triggers the commented early-out assert in BeginTabBarEx() (probably dock node
                        // going from implicit to explicit in same frame)

    if (!window->HasCloseButton)
        tab_flags |= ImGuiTabItemFlags_NoCloseButton; // Set _NoCloseButton immediately because it will be used for
                      ...
                      ImGuiWindow *docked_window)
{
    ...
    if (want_clip_rect)
        PushClipRect(ImVec2(ImClamp(bb.Min.x, tab_bar->ScrollingRectMinX, tab_bar->ScrollingRectMaxX), bb.Min.y - 1),
    ...
    ImGuiButtonFlags button_flags =
        ((ImGuiButtonFlags)(is_tab_button ? ImGuiButtonFlags_PressedOnClickRelease : ImGuiButtonFlags_PressedOnClick) |
         ImGuiButtonFlags_AllowOverlap);
    if (g.DragDropActive &&
        !g.DragDropPayload.IsDataType(
            IMGUI_PAYLOAD_TYPE_WINDOW)) // FIXME: May be an opt-in property of the payload to disable this
        button_flags |= ImGuiButtonFlags_PressedOnDragDropHold;
    ...
    else if (held && !tab_appearing && IsMouseDragging(0))
    {
        ...
        const bool can_undock = docked_window != NULL && !(docked_window->Flags & ImGuiWindowFlags_NoMove) &&
                                !(node->MergedFlags & ImGuiDockNodeFlags_NoUndocking);
        if (can_undock)
        {
            ...
            if (!undocking_tab) //&& (!g.IO.ConfigDockingWithShift || g.IO.KeyShift)
            {
                ...
                // GetForegroundDrawList()->AddRect(ImVec2(bb.Min.x - threshold_x, bb.Min.y - threshold_y),
                // ImVec2(bb.Max.x + threshold_x, bb.Max.y + threshold_y), IM_COL32_WHITE); // [DEBUG]
                ...
                if (drag_distance_from_edge_x > threshold_x)
                    if ((drag_dir < 0 && TabBarGetTabOrder(tab_bar, tab) == 0) ||
                        (drag_dir > 0 && TabBarGetTabOrder(tab_bar, tab) == tab_bar->Tabs.Size - 1))
                        undocking_tab = true;
            }
    ...
    if (is_visible)
    {
        ...
        const ImGuiID close_button_id =
            p_open ? GetIDWithSeed("#CLOSE", NULL, docked_window ? docked_window->ID : id) : 0;
        bool just_closed;
        bool text_clipped;
        ...
        // Forward Hovered state so IsItemHovered() after Begin() can work (even though we are technically hovering our
        // parent) That state is copied to window->DockTabItemStatusFlags by our caller.
                                       ...
                                       bool is_contents_visible, bool *out_just_closed, bool *out_text_clipped)
{
    ...
    LogSetNextTextDecoration("/", "\\");
    RenderTextEllipsis(draw_list, text_ellipsis_clip_bb.Min, text_ellipsis_clip_bb.Max, ellipsis_max_x, label,
