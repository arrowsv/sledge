// Edited for sRGB - Line 531

// dear imgui: Renderer Backend for DirectX11
// This needs to be used along with a Platform Backend (e.g. Win32)

// Implemented features:
//  [X] Renderer: User texture binding. Use 'ID3D11ShaderResourceView*' as texture identifier. Read the FAQ about
...
//  (ImGuiBackendFlags_RendererHasTextures). [X] Renderer: Expose selected render state for draw callbacks to use.
//  Access in '(ImGui_ImplXXXX_RenderState*)GetPlatformIO().Renderer_RenderState'. [X] Renderer: Multi-viewport support
...
//  2026-04-23: DirectX11: Added support for standard draw callbacks (in platform_io): DrawCallback_ResetRenderState,
//  DrawCallback_SetSamplerLinear, DrawCallback_SetSamplerNearest. Obsoleting samplers from ImGui_ImplDX11_RenderState.
//  (#9378) 2026-01-19: DirectX11: Added 'SamplerNearest' in ImGui_ImplDX11_RenderState. Renamed 'SamplerDefault' to
//  'SamplerLinear'. 2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown. 2025-06-11: DirectX11: Added
//  support for ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. 2025-05-07: DirectX11: Honor
//  draw_data->FramebufferScale to allow for custom backends and experiment using it (consistently with other renderer
//  backends, even though in normal condition it is not set under Windows). 2025-02-24: [Docking] Added undocumented
//  ImGui_ImplDX11_SetSwapChainDescs() to configure swap chain creation for secondary viewports. 2025-01-06: DirectX11:
//  Expose VertexConstantBuffer in ImGui_ImplDX11_RenderState. Reset projection matrix in
//  ImDrawCallback_ResetRenderState handler. 2024-10-07: DirectX11: Changed default texture sampler to Clamp instead of
//  Repeat/Wrap. 2024-10-07: DirectX11: Expose selected render state in ImGui_ImplDX11_RenderState, which you can access
//  in 'void* platform_io.Renderer_RenderState' during draw callbacks. 2022-10-11: Using 'nullptr' instead of 'NULL' as
//  per our switch to C++11. 2021-06-29: Reorganized backend to pull data from a single structure to facilitate usage
//  with multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-05-19: DirectX11: Replaced direct access to
//  ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18: DirectX11: Change
//  blending equation to preserve alpha in output buffer. 2019-08-01: DirectX11: Fixed code querying the Geometry Shader
//  state (would generally error with Debug layer enabled). 2019-07-21: DirectX11: Backup, clear and restore Geometry
//  Shader is any is bound when calling ImGui_ImplDX11_RenderDrawData. Clearing Hull/Domain/Compute shaders without
//  backup/restore. 2019-05-29: DirectX11: Added support for large mesh (64K+ vertices), enable
...
//  so it can be displayed in the About Window. 2018-08-01: DirectX11: Querying for IDXGIFactory instead of
//  IDXGIFactory1 to increase compatibility. 2018-07-13: DirectX11: Fixed unreleased resources in Init and Shutdown
//  functions. 2018-06-08: Misc: Extracted imgui_impl_dx11.cpp/.h away from the old combined DX11+Win32 example.
//  2018-06-08: DirectX11: Use draw_data->DisplayPos and draw_data->DisplaySize to setup projection matrix and clipping
//  rectangle. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback and exposed ImGui_ImplDX11_RenderDrawData()
//  in the .h file so you can call it yourself. 2018-02-06: Misc: Removed call to ImGui::Shutdown() which is not
//  available from 1.60 WIP, user needs to call CreateContext/DestroyContext themselves. 2016-05-07: DirectX11:
...
struct ImGui_ImplDX11_Data
{
    ...
    ImGui_ImplDX11_RenderState
        *RenderState; // == (ImGui_ImplDX11_RenderState*)ImGui::GetPlatformIO().Renderer_RenderState during rendering.
...
static void ImGui_ImplDX11_DrawCallback_SetSamplerLinear(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplDX11_Data *bd = ImGui_ImplDX11_GetBackendData();
    bd->RenderState->DeviceContext->PSSetSamplers(0, 1, &bd->pTexSamplerLinear);
}
static void ImGui_ImplDX11_DrawCallback_SetSamplerNearest(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplDX11_Data *bd = ImGui_ImplDX11_GetBackendData();
    bd->RenderState->DeviceContext->PSSetSamplers(0, 1, &bd->pTexSamplerNearest);
}
...
void ImGui_ImplDX11_UpdateTexture(ImTextureData *tex)
{
    ...
    else if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        for (ImTextureRect &r : tex->Updates)
        {
            ...
            bd->pd3dDeviceContext->UpdateSubresource(backend_tex->pTexture, 0, &box, tex->GetPixelsAt(r.x, r.y),
                                                     (UINT)tex->GetPitch(), 0);
        }
        tex->SetStatus(ImTextureStatus_OK);
    }
...
bool ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        D3D11_INPUT_ELEMENT_DESC local_layout[] = {
            {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0, (UINT)offsetof(ImDrawVert, pos), D3D11_INPUT_PER_VERTEX_DATA,
             0},
            {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, (UINT)offsetof(ImDrawVert, uv), D3D11_INPUT_PER_VERTEX_DATA,
             0},
            {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, (UINT)offsetof(ImDrawVert, col), D3D11_INPUT_PER_VERTEX_DATA,
             0},
        };
...
void ImGui_ImplDX11_InvalidateDeviceObjects()
{
    ...
    if (bd->pTexSamplerNearest)
    {
        bd->pTexSamplerNearest->Release();
        bd->pTexSamplerNearest = nullptr;
    }
    if (bd->pIB)
    {
        bd->pIB->Release();
...
static void ImGui_ImplDX11_CreateWindow(ImGuiViewport *viewport)
{
    ...
    // GLFWWindow*, SDL's WindowID). Some backends will leave PlatformHandleRaw == 0, in which case we assume
    ...
    {
        IM_ASSERT(sd_template.BufferDesc.Width == 0 && sd_template.BufferDesc.Height == 0 &&
    ...
    bd->pFactory->MakeWindowAssociation(hwnd,
                                        DXGI_MWA_NO_ALT_ENTER | DXGI_MWA_NO_WINDOW_CHANGES); // Disable e.g. Alt+Enter
