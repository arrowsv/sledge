...
//  key functions e.g. ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy VK_* values are obsolete since 1.87 and not
//  supported since 1.91.5] [X] Platform: Gamepad support. [X] Platform: Mouse cursor shape and visibility
...
// #define IMGUI_IMPL_WIN32_DISABLE_GAMEPAD              // Disable gamepad support. This was meaningful before <1.81
// but we now load XInput dynamically so the option is now less relevant.

// CHANGELOG
// (minor and older changes stripped away, please see git history for details)
//  2026-XX-XX: Platform: Added support for multiple windows via the ImGuiPlatformIO interface.
//  2026-05-19: DPI: ImGui_ImplWin32_EnableDpiAwareness() helper uses SetProcessDpiAwarenessContext() instead of
//  SetThreadDpiAwarenessContext(), fixes DPI scaling issues with e.g. OpenGL. (#9403) 2026-03-31: [Viewports] Fixed
//  setting/getting size when viewports have OS decorations (e.g. io.ConfigViewportsNoDecoration = false) and process is
//  running in Per-Monitor V2 DPI mode. (#8897) 2026-01-28: Inputs: Minor optimization not submitting gamepad input if
//  packet number has not changed (reworked from 2025-09-23 attempt). (#9202, #8556) 2026-01-26: [Viewports] Fixed an
//  issue from 1.90.5 where newly appearing windows that are not parented to the main viewport don't have task bar icon
//  appear before the windows was explicited refocused. (#7354, #8669) 2025-12-03: Inputs: handle
//  WM_IME_CHAR/WM_IME_COMPOSITION messages to support Unicode inputs on MBCS (non-Unicode) Windows. (#9099, #3653,
//  #5961) 2025-10-19: Inputs: Revert previous change to allow for io.ClearInputKeys() on focus-out not losing gamepad
//  state. 2025-09-23: Inputs: Minor optimization not submitting gamepad input if packet number has not changed.
//  2025-09-18: Call platform_io.ClearPlatformHandlers() on shutdown.
//  2025-06-02: [Viewports] WM_DPICHANGED also apply io.ConfigDpiScaleViewports for main viewport instead of letting it
//  be done by application code. 2025-04-30: Inputs: Fixed an issue where externally losing mouse capture (due to e.g.
//  focus loss) would fail to claim it again the next subsequent click. (#8594) 2025-03-26: [Viewports] Viewports: fixed
//  an issue when closing a window from the OS close button (with io.ConfigViewportsNoDecoration = false) while user
//  code was discarding the 'bool* p_open = false' output from Begin(). Because we allowed the Win32 window to close
//  early, Windows destroyed it and our imgui window became not visible even though user code was still submitting it.
//  2025-03-10: When dealing with OEM keys, use scancodes instead of translated keycodes to choose ImGuiKey values.
//  (#7136, #7201, #7206, #7306, #7670, #7672, #8468) 2025-02-21: [Viewports] WM_SETTINGCHANGE's SPI_SETWORKAREA message
//  also triggers a refresh of monitor list. (#8415) 2025-02-18: Added ImGuiMouseCursor_Wait and
//  ImGuiMouseCursor_Progress mouse cursor support. 2024-11-21: [Viewports] Fixed a crash when multiple processes are
//  running with multi-viewports, caused by misusage of GetProp(). (#8162, #8069) 2024-10-28: [Viewports] Rely on
//  property stored inside HWND to retrieve context/viewport, should facilitate attempt to use this for parallel
//  contexts. (#8069) 2024-09-16: [Viewports] Inputs: fixed an issue where a viewport destroyed while clicking would hog
//  mouse tracking and temporary lead to incorrect update of HoveredWindow. (#7971) 2024-07-08: Inputs: Fixed
//  ImGuiMod_Super being mapped to VK_APPS instead of VK_LWIN||VK_RWIN. (#7768) 2023-10-05: Inputs: Added support for
//  extra ImGuiKey values: F13 to F24 function keys, app back/forward keys. 2023-09-25: Inputs: Synthesize key-down
//  event on key-up for VK_SNAPSHOT / ImGuiKey_PrintScreen as Windows doesn't emit it (same behavior as GLFW/SDL).
//  2023-09-07: Inputs: Added support for keyboard codepage conversion for when application is compiled in MBCS mode and
//  using a non-Unicode window. 2023-04-19: Added ImGui_ImplWin32_InitForOpenGL() to facilitate combining raw
//  Win32/Winapi with OpenGL. (#3218) 2023-04-04: Inputs: Added support for io.AddMouseSourceEvent() to discriminate
//  ImGuiMouseSource_Mouse/ImGuiMouseSource_TouchScreen/ImGuiMouseSource_Pen. (#2702) 2023-02-15: Inputs: Use
//  WM_NCMOUSEMOVE / WM_NCMOUSELEAVE to track mouse position over non-client area (e.g. OS decorations) when app is not
//  focused. (#6045, #6162) 2023-02-02: Inputs: Flipping WM_MOUSEHWHEEL (horizontal mouse-wheel) value to match other
//  backends and offer consistent horizontal scrolling direction. (#4019, #6096, #1463) 2022-10-11: Using 'nullptr'
//  instead of 'NULL' as per our switch to C++11. 2022-09-28: Inputs: Convert WM_CHAR values with MultiByteToWideChar()
//  when window class was registered as MBCS (not Unicode). 2022-09-26: Inputs: Renamed ImGuiKey_ModXXX introduced
//  in 1.87 to ImGuiMod_XXX (old names still supported). 2022-01-26: Inputs: replaced short-lived io.AddKeyModsEvent()
//  (added two weeks ago) with io.AddKeyEvent() using ImGuiKey_ModXXX flags. Sorry for the confusion. 2021-01-20:
//  Inputs: calling new io.AddKeyAnalogEvent() for gamepad support, instead of writing directly to io.NavInputs[].
//  2022-01-17: Inputs: calling new io.AddMousePosEvent(), io.AddMouseButtonEvent(), io.AddMouseWheelEvent() API
//  (1.87+). 2022-01-17: Inputs: always update key mods next and before a key event (not in NewFrame) to fix input queue
//  with very low framerates. 2022-01-12: Inputs: Update mouse inputs using WM_MOUSEMOVE/WM_MOUSELEAVE + fallback to
//  provide it when focused but not hovered/captured. More standard and will allow us to pass it to future input queue
//  API. 2022-01-12: Inputs: Maintain our own copy of MouseButtonsDown mask instead of using ImGui::IsAnyMouseDown()
//  which will be obsoleted. 2022-01-10: Inputs: calling new io.AddKeyEvent(), io.AddKeyModsEvent() +
//  io.SetKeyEventNativeData() API (1.87+). Support for full ImGuiKey range. 2021-12-16: Inputs: Fill
//  VK_LCONTROL/VK_RCONTROL/VK_LSHIFT/VK_RSHIFT/VK_LMENU/VK_RMENU for completeness. 2021-08-17: Calling
//  io.AddFocusEvent() on WM_SETFOCUS/WM_KILLFOCUS messages. 2021-08-02: Inputs: Fixed keyboard modifiers being reported
//  when host window doesn't have focus. 2021-07-29: Inputs: MousePos is correctly reported when the host platform
//  window is hovered but not focused (using TrackMouseEvent() to receive WM_MOUSELEAVE events). 2021-06-29: Reorganized
//  backend to pull data from a single structure to facilitate usage with multiple-contexts (all g_XXXX access changed
//  to bd->XXXX). 2021-06-08: Fixed ImGui_ImplWin32_EnableDpiAwareness() and ImGui_ImplWin32_GetDpiScaleForMonitor() to
//  handle Windows 8.1/10 features without a manifest (per-monitor DPI, and properly calls SetProcessDpiAwareness()
//  on 8.1). 2021-03-23: Inputs: Clearing keyboard down array when losing focus (WM_KILLFOCUS). 2021-02-18: Added
...
//  dragging. 2016-11-12: Inputs: Only call Win32 ::SetCursor(nullptr) when io.MouseDrawCursor is set.
...
static bool ImGui_ImplWin32_InitEx(void *hwnd, bool platform_has_own_dc)
{
    ...
    io.BackendFlags |= ImGuiBackendFlags_HasMouseHoveredViewport; // We can call io.AddMouseViewportEvent() with correct
                                                                  // data (optional)
    io.BackendFlags |=
        ImGuiBackendFlags_HasParentViewport; // We can honor viewport->ParentViewportId by applying the corresponding
    ...
    // So as we store a pointer in IMGUI_CONTEXT we need to make sure we only call GetPropA() on windows owned by our
    // process.
    ...
    if (HINSTANCE user32_dll = ::GetModuleHandleA("user32.dll"))
        bd->AdjustWindowRectExForDpi =
            (PFN_AdjustWindowRectExForDpi)::GetProcAddress(user32_dll, "AdjustWindowRectExForDpi");
...
static void ImGui_ImplWin32_UpdateMouseData(ImGuiIO &io, ImGuiPlatformIO &platform_io)
{
    ...
    const bool is_app_focused =
        (focused_window && (focused_window == bd->hWnd || ::IsChild(focused_window, bd->hWnd) ||
                            ImGui_ImplWin32_FindViewportByPlatformHandle(platform_io, focused_window)));
    if (is_app_focused)
    {
        // (Optional) Set OS mouse position from Dear ImGui if requested (rarely used, only when
        // io.ConfigNavMoveSetMousePos is enabled by user) When multi-viewports are enabled, all Dear ImGui positions
        // are same as OS positions.
        ...
        // or captured) This also fills a short gap when clicking non-client area: WM_NCMOUSELEAVE -> modal OS move ->
        // gap -> WM_NCMOUSEMOVE
        if (!io.WantSetMousePos && bd->MouseTrackedArea == 0 && has_mouse_screen_pos)
        {
            ...
            // on the upper-left corner of the app window) (This is the position you can get with ::GetCursorPos() +
            // ::ScreenToClient() or WM_MOUSEMOVE.) Multi-viewport mode: mouse position in OS absolute coordinates
            // (io.MousePos is (0,0) when the mouse is on the upper-left of the primary monitor) (This is the position
            // you can get with ::GetCursorPos() or WM_MOUSEMOVE + ::ClientToScreen(). In theory adding viewport->Pos to
            // a client position would also be the same.)
    ...
    // (Optional) When using multiple viewports: call io.AddMouseViewportEvent() with the viewport the OS mouse cursor
    // is hovering. If ImGuiBackendFlags_HasMouseHoveredViewport is not set by the backend, Dear imGui will ignore this
    // field and infer the information using its flawed heuristic.
    // - [X] Win32 backend correctly ignore viewports with the _NoInputs flag (here using ::WindowFromPoint with
    // WM_NCHITTEST + HTTRANSPARENT in WndProc does that)
    ...
    // - [X] Win32 backend correctly reports this regardless of another viewport behind focused and dragged from (we
...
ImGuiKey ImGui_ImplWin32_KeyEventToImGuiKey(WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case 41:
        return ImGuiKey_GraveAccent; // VK_OEM_8 in EN-UK, VK_OEM_3 in EN-US, VK_OEM_7 in FR, VK_OEM_5 in DE, etc.
    case 12:
...
// Prefer to call this at the top of the message handler to avoid the possibility of other Win32 calls interfering with
// this.
...
// from your application based on those two flags. PS: We treat DBLCLK messages as regular mouse down messages, so this
// code will work on windows classes that have the CS_DBLCLKS flag set. Our own example app code doesn't set this flag.
...
{
    // Most backends don't have silent checks like this one, but we need it because WndProc are called early in
    // CreateWindow(). We silently allow both context or just only backend data to be nullptr.
...
#ifndef IMGUI_IMPL_WIN32_DISABLE_GAMEPAD
        bd->XInputPacketNumber =
            0; // FIXME: Technically, calling io.ClearInputKeys() directly would require this as well.
#endif
        return 0;
...
//   runtime, neither of which we want to require the user to have. So we dynamically select and load those functions to
//   avoid dependencies.
//---------------------------------------------------------------------------------------------------------
...
typedef BOOL(WINAPI *PFN_SetProcessDpiAwarenessContext)(
    DPI_AWARENESS_CONTEXT); // User32.lib + dll, Windows 10 v1703+ (Creators Update)
...
void ImGui_ImplWin32_EnableDpiAwareness()
{
    ...
    if (_IsWindows10OrGreater())
    {
        ...
        if (PFN_SetProcessDpiAwarenessContext SetProcessDpiAwarenessContextFn =
                (PFN_SetProcessDpiAwarenessContext)::GetProcAddress(
                    user32_dll, "SetProcessDpiAwarenessContext")) // Windows 10 v1703+
        {
        ...
        if (PFN_SetThreadDpiAwarenessContext SetThreadDpiAwarenessContextFn =
                (PFN_SetThreadDpiAwarenessContext)::GetProcAddress(user32_dll,
                                                                   "SetThreadDpiAwarenessContext")) // Windows 10 v1607+
        {
...
static bool ImGui_ImplWin32_IsDpiAwarenessPerMonitorAwareV2()
{
    ...
    PFN_GetThreadDpiAwarenessContext GetThreadDpiAwarenessContextFn =
        (PFN_GetThreadDpiAwarenessContext)::GetProcAddress(user32_dll, "GetThreadDpiAwarenessContext");
    PFN_AreDpiAwarenessContextsEqual AreDpiAwarenessContextsEqualFn =
        (PFN_AreDpiAwarenessContextsEqual)::GetProcAddress(user32_dll, "AreDpiAwarenessContextsEqual");
    if (GetThreadDpiAwarenessContextFn && AreDpiAwarenessContextsEqualFn)
        return AreDpiAwarenessContextsEqualFn(GetThreadDpiAwarenessContextFn(),
...
// Helper structure we store in the void* PlatformUserData field of each ImGuiViewport to easily retrieve our backend
// data.
...
static void ImGui_ImplWin32_SetWindowSize(ImGuiViewport *viewport, ImVec2 size)
{
    ...
    if (viewport->Flags & ImGuiViewportFlags_OwnedByApp)
        ImGui_ImplWin32_UpdateWin32StyleFromWindow(viewport); // Not our window, poll style before using
    ImGui_ImplWin32_AdjustWindowRect(viewport, &rect, vd->DwStyle, vd->DwExStyle); // Client to Screen
    ::SetWindowPos(vd->Hwnd, nullptr, 0, 0, rect.right - rect.left, rect.bottom - rect.top,
...
static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (ctx == NULL)
        return ::DefWindowProcW(hWnd, msg, wParam, lParam); // unlike ImGui_ImplWin32_WndProcHandler() we are called
                                                            // directly by Windows, we can't just return 0.
    ...
    else if (ImGuiViewport *viewport = ImGui_ImplWin32_FindViewportByPlatformHandle(platform_io, hWnd))
    {
        ...
        case WM_MOUSEACTIVATE:
            ...
            break;
        case WM_NCHITTEST:
            // Let mouse pass-through the window. This will allow the backend to call io.AddMouseViewportEvent()
            // correctly. (which is optional). The ImGuiViewportFlags_NoInputs flag is set while dragging a viewport, as
            // want to detect the window behind the one we are dragging. If you cannot easily access those viewport
            // flags from your windowing/event code: you may manually synchronize its state e.g. in your main loop after
            // calling UpdatePlatformWindows(). Iterate all viewports/platform windows and pass the flag to your
            // windowing system.
