                         ...
                         const ImVec2 &outer_size, float inner_width)
{
    ...
    const ImVec2 actual_outer_size =
    ...
    else
    {
        ...
        table->InnerWindow->DC
            .TreeDepth++; // This is designed to always linking ImGuiTreeNodeFlags_DrawLines linking across a table
    }
...
void ImGui::TableSetColumnDisplayOrder(ImGuiTable *table, int column_n, int dst_order)
{
    ...
    // Display order is stored in both columns->IndexDisplayOrder and table->DisplayOrder[]. Rebuild later from the
    // former.
...
static int TableGetMaxDisplayOrderAllowed(ImGuiTable *table, int src_order, int dst_order)
{
    ...
    // TableSetupScrollFreeze() enforce a display order range for frozen columns. Reordering across the frozen column
    // barrier is illegal and will be undone.
    if (table->FreezeColumnsRequest > 0)
        dst_order = (src_order < table->FreezeColumnsRequest) ? ImMin(dst_order, (int)table->FreezeColumnsRequest - 1)
                                                              : ImMax(dst_order, (int)table->FreezeColumnsRequest);

    // Cannot cross over a column with the ImGuiTableColumnFlags_NoReorder flag.
    int reorder_dir = (src_order < dst_order) ? +1 : -1;
    for (int order_n = src_order;
         (src_order < dst_order && order_n <= dst_order) || (dst_order < src_order && order_n >= dst_order);
         order_n += reorder_dir)
...
void ImGui::TableUpdateLayout(ImGuiTable *table)
{
    ...
    // Else if give no chance to a clipper-savvy user to submit rows and therefore total contents height used by
    // scrollbar.
    ...
    if (table->FreezeColumnsRequest > 0)
        table->InnerWindow->DecoInnerSizeX1 =
            ...
            table->OuterRect.Min.x; // FIXME-FROZEN
...
void ImGui::EndTable()
{
    ...
    const float inner_content_max_y =
        ImCeil(table->RowPosY2); // Rounding final position is important as we currently don't round row height
                                 // ('Demo->Tables->Outer Size' demo uses non-integer heights)
    ...
    {
        // Some references for this: #7651 + tests "table_reported_size", "table_reported_size_outer" equivalent Y
        // block, #9352
        // - FIXME-TABLE: Would make sense to pre-compute expected scrollbar visibility/sizes to generally save a frame
        // of feedback? See broken test in 'table_reported_size_outer'
        const float outer_content_max_x = table->OuterRect.Min.x + table->ColumnsAutoFitWidth;
        const float decoration_size = table->TempData->AngledHeadersExtraWidth +
                                      ((inner_window != outer_window) ? inner_window->ScrollbarSizes.x : 0.0f);
        outer_window->DC.IdealMaxPos.x =
            ImMax(outer_window->DC.IdealMaxPos.x, outer_content_max_x + decoration_size - temp_data->UserOuterSize.x);
        outer_window->DC.CursorMaxPos.x =
            ImMax(backup_outer_max_pos.x, ImMin(table->OuterRect.Max.x, outer_content_max_x + decoration_size));
    }
    else
    {
    ...
    if (temp_data->UserOuterSize.y <= 0.0f)
    {
        ...
        const float outer_content_size_y = (inner_window == outer_window)
        ...
        outer_window->DC.IdealMaxPos.y =
            ImMax(outer_window->DC.IdealMaxPos.y, outer_content_max_y + decoration_size - temp_data->UserOuterSize.y);
        outer_window->DC.CursorMaxPos.y =
            ImMax(backup_outer_max_pos.y, ImMin(table->OuterRect.Max.y, outer_content_max_y + decoration_size));
    }
    else
    {
    ...
    g.CurrentTable =
        temp_data && (temp_data->WindowID == outer_window->ID) ? g.Tables.GetByIndex(temp_data->TableIndex) : NULL;
    if (g.CurrentTable)
    {
                             ...
                             ImGuiID user_id)
{
    ...
    IM_ASSERT_USER_ERROR_RET(table->DeclColumnsCount < table->ColumnsCount,
                             "TableSetupColumn(): called too many times!");
    IM_ASSERT_USER_ERROR_RET(
        table->IsLayoutLocked == false,
        "TableSetupColumn(): need to call before first row!"); // Table layout is locked when submitting a row or when
                                                               // calling BeginMultiSelect() with box-select.
    ...
    if (table->IsDefaultSizingPolicy && (flags & ImGuiTableColumnFlags_WidthMask_) == 0 &&
        ...
        IM_ASSERT_USER_ERROR_RET(init_width_or_weight <= 0.0f,
                                 "TableSetupColumn(): can only specify width/weight if sizing policy is set explicitly "
                                 "in either Table or Column.");
...
bool ImGui::TableSetColumnIndex(int column_n)
{
    ...
    if (table->CurrentColumn != column_n)
    {
        if (table->CurrentColumn != -1)
            TableEndCell(table);
        IM_ASSERT_USER_ERROR_RETV(column_n >= 0 && column_n < table->ColumnsCount, false,
...
void ImGui::TableDrawBorders(ImGuiTable *table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (is_frozen_separator)
                ...
                draw_y2 = draw_y2_body;
            else if ((table->Flags & (ImGuiTableFlags_NoBordersInBodyUntilResize | ImGuiTableFlags_NoBordersInBody)) ==
                     0)
                draw_y2 = draw_y2_body;
            if (draw_y2 > draw_y1)
                inner_drawlist->AddLineV(column->MaxX, draw_y1, draw_y2,
    ...
    if (table->Flags & ImGuiTableFlags_BordersOuter)
    {
        ...
        {
            inner_drawlist->AddLineV(outer_border.Min.x, outer_border.Min.y, outer_border.Max.y, outer_col,
                                     border_size);
            inner_drawlist->AddLineV(outer_border.Max.x, outer_border.Min.y, outer_border.Max.y, outer_col,
                                     border_size);
        }
        else if (table->Flags & ImGuiTableFlags_BordersOuterH)
        {
            inner_drawlist->AddLineH(outer_border.Min.x, outer_border.Max.x, outer_border.Min.y, outer_col,
                                     border_size);
            inner_drawlist->AddLineH(outer_border.Min.x, outer_border.Max.x, outer_border.Max.y, outer_col,
                                     border_size);
        }
    }
                                    ...
                                    const ImGuiTableHeaderData *data, int data_count)
{
    ...
    if (table->FreezeColumnsCount > 0)
        clip_rect_min_x =
            ImMax(clip_rect_min_x, table->Columns[table->DisplayOrderToIndex[table->FreezeColumnsCount - 1]].MaxX);
    ...
    for (int pass = 0; pass < 2; pass++)
        for (int order_n = 0; order_n < data_count; order_n++)
        {
            ...
            if (pass == 0)
            {
                ...
                float line_off_for_align_x = ImFloor(
...
static bool MenuItemForColumnReorder(const char *label, bool selected, bool enabled)
{
    ...
    bool ret = Selectable(label, false, selectable_flags, ImVec2(min_w, label_size.y)) &&
    ...
    if ((g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) && selected)
        RenderCheckMark(window->DrawList,
                        text_pos +
                            ImVec2(offsets->OffsetMark + stretch_w + g.FontSize * 0.40f, g.FontSize * 0.134f * 0.5f),
...
void ImGui::TableDrawDefaultContextMenu(ImGuiTable *table, ImGuiTableFlags flags_for_section_to_display)
{
    ...
    const int context_column_n = (table->ContextPopupColumn >= 0 && table->ContextPopupColumn < table->ColumnsCount)
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Resizable)
    {
        if (context_column != NULL)
        {
            const bool can_resize =
                !(context_column->Flags & ImGuiTableColumnFlags_NoResize) && context_column->IsEnabled;
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Hideable)
    {
        ...
        // (because the query itself does a sweep scan). This assume that reordering constraints output a single range,
        // otherwise would need to either call TableGetMaxDisplayOrderAllowed() for each item below, or cache this once
        // per frame into columns.
        const bool is_reordering =
            (g.ActiveId != 0 && g.ActiveIdWindow == g.CurrentWindow && table->ReorderColumn != -1 &&
             g.ActiveIdHasBeenEditedBefore); // FIXME: This is a bit of a hack.
        ...
        const int reorder_max_order =
            is_reordering ? TableGetMaxDisplayOrderAllowed(table, reorder_src_order, table->ColumnsCount - 1)
        ...
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (IsItemActive() && IsMouseDragging(0) && g.ActiveIdSource == ImGuiInputSource_Mouse &&
                (table->Flags & ImGuiTableFlags_Reorderable))
            {
                g.ActiveIdHasBeenEditedBefore = true; // Disable toggle in MenuItemForColumnReorder() + start dimming to
                                                      // display allowed reorder targets.
                table->ReorderColumn = (ImGuiTableColumnIdx)column_n;
                if (!IsItemHovered())
                {
                    int reorder_dir =
                        (g.IO.MousePos.y < (g.LastItemData.Rect.Min.y + g.LastItemData.Rect.Max.y) * 0.5f) ? -1 : +1;
                    float reorder_amount = (reorder_dir < 0 ? g.LastItemData.Rect.Min.y - g.IO.MousePos.y
                                                            : g.IO.MousePos.y - g.LastItemData.Rect.Max.y) /
                                           g.LastItemData.Rect.GetHeight();
                    int dst_order = column->DisplayOrder +
                                    (int)ImCeil(reorder_amount) *
                                        reorder_dir; // Estimated target order, will be validated and clamped.
...
// Fix invalid display order data: compact values (0,1,3 -> 0,1,2); preserve relative order (0,3,1 -> 0,2,1);
// deduplicate (0,4,1,1 -> 0,3,1,2)
void ImGui::TableFixDisplayOrder(ImGuiTable *table)
{
    ImGuiContext &g = *GImGui;
    g.TempBuffer.reserve((int)(sizeof(ImGuiTableFixDisplayOrderColumnData) *
                               table->ColumnsCount)); // FIXME: Maybe wrap those two lines as a helper.
    ...
    ImQsort(fdo_columns, (size_t)table->ColumnsCount, sizeof(ImGuiTableFixDisplayOrderColumnData),
            TableFixDisplayOrderComparer);
    for (int n = 0; n < table->ColumnsCount; n++)
...
void ImGui::DebugNodeTable(ImGuiTable *table)
{
    ...
    if (IsItemHovered())
        GetForegroundDrawList(table->OuterWindow)
            ->AddRect(table->OuterRect.Min, table->OuterRect.Max, IM_COL32(255, 255, 0, 255));
    if (IsItemVisible() && table->HoveredColumnBody != -1)
        GetForegroundDrawList(table->OuterWindow)
            ->AddRect(GetItemRectMin(), GetItemRectMax(), IM_COL32(255, 255, 0, 255));
    if (!open)
        return;
    if (table->InstanceCurrent > 0)
    ...
    BulletText("ResizedColumn: %d, HeldHeaderColumn: %d, ReorderColumn: %d", table->LastResizedColumn,
               table->LastHeldHeaderColumn, table->ReorderColumn);
